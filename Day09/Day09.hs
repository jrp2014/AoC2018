...
turn marble circle
  | marble `mod` 23 == 0 =
    (toAdd + marble, fromJust (deleteRight shiftedCircle))
  | otherwise = (0, (insertLeft marble . shiftFocusN 2) circle)
  where
    shiftedCircle = shiftFocusN (-7) circle
    toAdd = current shiftedCircle

game :: Int -> Int -> ([Score], Circle)
game nPlayers nMarbles = foldl' newScore (initialScores, initialCircle) turns
  where
    newScore (scores, circle) (player, marble) =
      (changeNthElement player (+ score) scores, circle')
      where
        (score, circle') = turn marble circle
    players = cycle [1 .. nPlayers]
    toAdd = [1 .. nMarbles]
    turns = zip players toAdd
